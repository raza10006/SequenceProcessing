...
The implementation lives in `src/main/java/SequenceProcessing/Classification/Bert.java`, with hyperparameters in `src/main/java/SequenceProcessing/Parameters/BertParameter.java` (which extends `TransformerParameter`, inheriting shared hyperparameters such as `L`, `V`, `N`, `dk`, and the optimizer/loss settings rather than duplicating them on `NeuralNetworkParameter`). The encoder graph is built end-to-end in `Bert#train`:
